...
| `POST /consent-request/{id}/tool-result` | Side-channel endpoint the frontend POSTs to after executing an on-device tool. Body: `{ tool_call_id, status: "ok" \| "error" \| "permission_denied", result? }`. **Dual-mode response.** If a parked SSE for this consent still exists in backend memory (within the 20s in-stream cap), this POST signals the agent's `asyncio.Event` and returns JSON 200 — the original `/agent-turn` SSE delivers events. If no parked SSE exists (window elapsed, screen reopened, instance restart), this POST returns `text/event-stream`: the handler rehydrates the agent from the durable snapshot, hands the in-flight tool result to it via `DeferredToolResults`, and streams the resumed iteration's events on the SAME response. No separate `/agent-turn` POST is needed to resume. |
...
**Freshness guarantee (scoped to consent_request lifetime):** the consent component is a trust boundary, not a chat history store. The agent's working memory persists for the **lifetime of one consent_request only** — so that closing and reopening the screen mid-flow continues from where the owner left off (mid-OTP, mid-refinement, mid-permission-grant) — and is **deleted immediately** when the consent_request reaches any terminal status (`approved`, `declined`, `expired`, `canceled` — single-`l` matches the existing schema CHECK constraint) in the same transaction as the status update. A new consent_request always starts with a fresh agent with no memory of any prior session. Within-consent resumption is the new architectural capability; cross-consent isolation is the trust contract. Learning owner preferences across sessions is a future Component 7 (User Memory) concern, still out of scope here.

**Parked-wait lifecycle (bounded, heartbeated, connection-light):** for the short-wait happy path where the SSE remains open during a tool wait (mobile is actively connected, result expected to come back quickly), three rules govern resource usage:
- **Bounded duration** — every in-stream wait has a hard wall-clock cap of **20 seconds**. Within the cap, fast device tools (`search_contacts`, `get_calendar_events`, `get_messages`, `get_current_location`, `get_time`) almost always resolve in well under a second so the in-stream wakeup delivers their results with no SSE close. When the cap is reached (slow human-input tools like OTP, permission grants), the SSE closes silently — there is no synthetic `step: timeout` placeholder; the snapshot taken at the `tool_call` boundary already captured everything needed for the durable resume path to take over.
- **Periodic heartbeats** — the SSE stream emits a no-op keepalive comment every **4 seconds**, giving the route layer a write opportunity so closed mobile clients can be detected mid-wait, and letting the cancellation flag be observed without waiting for the cap.
- **Connection-light** — the route handler acquires a short-lived DB session for prep work and releases it before the SSE generator enters the agent loop.

**Long-wait lifecycle (durable agent state, no parking):** when a tool wait exceeds the in-stream cap, the agent does NOT park. Instead, at the boundary where the agent emits a `tool_call`, it persists its full working memory (Pydantic AI `message_history` + pending tool calls + iteration + UI render state) to a dedicated runtime store keyed by `consent_request_id`, and the SSE closes cleanly. The Cloud Run concurrency slot is released; no DB connection is held; no `asyncio.Event` is left waiting.

When the device delivers the tool result via `POST /tool-result`, the backend response is **dual-mode**: if a parked SSE for this consent still exists in backend memory (within the 20s window), the handler signals the parked agent's `asyncio.Event` and returns JSON 200 — the original SSE delivers the resumed iteration's events. If no parked SSE exists (window elapsed, instance restart, screen reopened), the same `/tool-result` POST returns a `text/event-stream` response: the handler holds the in-flight tool result in local memory, rehydrates the agent from the durable snapshot, hands the result to the rehydrated agent via `DeferredToolResults`, and streams the resumed iteration's events on the SAME response (closure events for the resolved tool first, then iteration N+1 — drafting tokens, `done`). Single round-trip, no separate `/agent-turn` POST is needed to resume after a tool result. From the agent's perspective it never paused; from the owner's perspective the screen continues as if the SSE never closed. The mobile, on screen reopen, fetches the UI render state and repaints the chat thread exactly as it was, including any pending OTP / permission-grant popup; if the snapshot's `has_pending_resume` flag indicates a tool result was POSTed but the previous SSE was killed before consuming it, mobile auto-fires a resume `/agent-turn`.

This composition — short-wait kept in-stream with a cap; long-wait checkpointed and resumed — guarantees that a single misbehaving consent flow can never cascade beyond one user retry, and that owner reaction times can be arbitrarily long without holding any backend resources during the wait. Backend availability for unrelated traffic is invariant of consent agent state.
