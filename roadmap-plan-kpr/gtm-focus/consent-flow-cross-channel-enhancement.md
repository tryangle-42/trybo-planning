...
- **Bounded duration** — every in-stream wait has a hard wall-clock cap (multiple minutes). If no result arrives within the cap, the wait surfaces a `step` SSE event with `status: "timeout"` and the agent continues with a `{status: "timeout"}` placeholder. Waits are never indefinite.
- **Periodic heartbeats** — the SSE stream emits a no-op keepalive comment at a fixed interval, giving the route layer a write opportunity so closed mobile clients can be detected mid-wait, and letting the cancellation flag be observed without waiting for the cap.
