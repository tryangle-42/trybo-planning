...
Filtering disconnected sources out of the agent's tool list would silently bypass the entire `permission_flow` mechanism. The way an owner gets prompted to connect Google Calendar is: the agent calls `google_calendar_list_events`, the adapter sees no connection row, the call returns a `needs_connection` envelope with the OAuth resolution payload, and the agent surfaces a connect prompt. If the tool is hidden, the agent never has the option to offer connecting it — it just falls back to device calendar forever. Connection state is therefore surfaced **at tool-call time via the envelope**, not by trimming the tool list — and not by annotating state in the system prompt either, since the envelope is the single source of truth and a prompt annotation would be a second, drift-prone copy.
...
Drill tools (`gmail_get_thread`, `googlecalendar_get_event`) take an id returned by a prior primary call (`gmail_search_emails`, `google_calendar_list_events`). They are follow-ups in a search → fetch sequence, not category-level alternatives. If they appeared in the preferred → fallback list, the agent would sometimes pick them as the primary lookup ("show me the email about X" → drill tool with no id → error or hallucinated id). The registry marks these entries with a `selection_role="drill"` flag, the renderer filters them out of the category block, and their tool descriptions carry an explicit "Do NOT call without a [thread_id|event_id] from [primary tool]" prerequisite phrase so the LLM still treats them correctly when called directly with an id.
